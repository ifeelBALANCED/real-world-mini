...
import { isSelectableItem } from "@/configs/enums/enumetable.abstract";
  ...
  Object.entries(values).forEach(([key, value]) => {
    ...
    if (value === undefined || value === null) {
      ...
      value.forEach((entry, i) => {
        if (
          Array.isArray(entry) ||
          (!isSelectableItem(entry) && entry === Object(entry))
        ) {
...
export const convertValuesToFormData = (
  ...
  return recursiveAppend(
    formName ? { [formName]: values } : values,
    new FormData()
  );
};
 ...
 * @param values
 */
export const convertValuesToObject = (
  values: Record<string, any>
): AnyObject | null => {
  if (!values) {
    return null;
